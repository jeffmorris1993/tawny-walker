...
export default function Photo({
  ...
  // Tripped when the WebP twin of a static photo is missing (someone added
  // a PNG without re-running `npm run photos`). See the onError handler.
  ...
  // Static photos under /photos/ are raw PNGs — thumbUrl() no-ops on them
  // because they aren't Supabase Storage URLs, so nothing else shrinks
  // them. Each ships a WebP twin generated by scripts/optimize-photos.mjs
  // (a 2.9 MB PNG becomes 139 KB), offered here through <picture> with the
  // PNG left as the fallback for browsers without WebP.
  const webpSrc = !webpFailed && /^\/photos\/[^/]+\.png$/.test(resolvedSrc || '')
    ? resolvedSrc.replace(/\.png$/, '.webp')
    : null;

  // Reset the fade-in and both fallback flags whenever src/width changes.
  // Skip straight to "loaded" if the browser already has the bytes —
    ...
    <div style={{
      ...
      {src && (
        <picture>
          ...
          <img
            // Keyed on the WebP fallback so dropping the <source> remounts
            // the element rather than relying on the browser to re-run
            // source selection against an unchanged src.
            key={webpSrc ? 'webp' : 'raw'}
            ref={imgRef}
            src={resolvedSrc}
            alt={label}
            ...
            onError={() => {
              // A <picture> commits to whichever <source> the browser
              // matched: if the .webp twin is missing it will NOT fall
              // back to the <img> on its own. Drop the source and
              // re-render against the PNG.
              if (webpSrc) {
                setWebpFailed(true);
                return;
              }
